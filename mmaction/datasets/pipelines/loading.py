...
class SampleFramesDiffPace:
    ...
    Args:
        clip_len (int): Frames of each sampled output clip.
        frame_interval_group (list): Temporal interval of adjacent sampled frames.
            Default: 1.
        num_clips (int): Number of clips to be sampled. Default: 1.
        temporal_jitter (bool): Whether to apply temporal jittering.
            Default: False.
    ...
    def __init__(self,
        ...
        self.clip_len = clip_len
        self.frame_interval_group = frame_interval_group
        self.num_clips = num_clips
        self.temporal_jitter = temporal_jitter
        self.twice_sample = twice_sample
        self.out_of_bound_opt = out_of_bound_opt
    ...
    def _get_train_clips(self, num_frames, frame_interval):
        """Get clip offsets in train mode.
    ...
    def _get_test_clips(self, num_frames, frame_interval):
        """Get clip offsets in test mode.
    ...
    def _sample_clips(self, num_frames, frame_interval):
        """Choose clip offsets for the video in a given mode.
        ...
        """
        if self.test_mode:
            clip_offsets = self._get_test_clips(num_frames, frame_interval)
        else:
            clip_offsets = self._get_train_clips(num_frames, frame_interval)
    ...
    def __call__(self, results):
        ...
        Args:
            results (dict): The resulting dict to be modified and passed
                to the next transform in pipeline.
        """
        frame_interval_choice = random.randint(0, len(self.frame_interval_group) - 1)
        frame_interval = self.frame_interval_group[frame_interval_choice]
        ...
        if self.frame_uniform:  # sthv2 sampling strategy
            ...
            clip_offsets = self._sample_clips(total_frames, frame_interval)
            frame_inds = clip_offsets[:, None] + np.arange(
                self.clip_len)[None, :] * frame_interval
            frame_inds = np.concatenate(frame_inds)

            if self.temporal_jitter:
                perframe_offsets = np.random.randint(
                    frame_interval, size=len(frame_inds))
                frame_inds += perframe_offsets
        ...
        results['num_clips'] = self.num_clips
        results['label'] = frame_interval_choice
        return results

    def __repr__(self):
        repr_str = (f'{self.__class__.__name__}('
                    f'clip_len={self.clip_len}, '
                    f'frame_interval_group={self.frame_interval_group}, '
                    f'num_clips={self.num_clips}, '
                    f'temporal_jitter={self.temporal_jitter}, '
                    f'twice_sample={self.twice_sample}, '
                    f'out_of_bound_opt={self.out_of_bound_opt}, '
